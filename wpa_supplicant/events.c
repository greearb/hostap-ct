...
static int wpa_supplicant_select_config(struct wpa_supplicant *wpa_s)
{
	...
	wpa_msg(wpa_s, MSG_DEBUG, "Select network based on association "
		"information");
	ssid = wpa_supplicant_get_ssid(wpa_s);
	if (ssid == NULL) {
		wpa_msg(wpa_s, MSG_INFO, "No network configuration found for the "
			"current AP");
		return -1;
	}

	if (ssid->disabled) {
		wpa_msg(wpa_s, MSG_DEBUG, "Selected network is disabled");
		return -1;
	}

	wpa_msg(wpa_s, MSG_DEBUG, "Network configuration found for the current "
		"AP");
...
static void wpa_find_assoc_pmkid(struct wpa_supplicant *wpa_s)
{
	...
	wpa_msg(wpa_s, MSG_DEBUG, "RSN: PMKID from assoc IE %sfound from PMKSA "
...
{
	if (data == NULL) {
		wpa_msg(wpa_s, MSG_DEBUG, "RSN: No data in PMKID candidate event");
		return;
	}
	wpa_msg(wpa_s, MSG_DEBUG, "RSN: PMKID candidate event - bssid=" MACSTR
		" index=%d preauth=%d",
			      ...
			      struct wpa_ssid *ssid)
{
	...
	if (!sim && !aka) {
		wpa_msg(wpa_s, MSG_DEBUG, "Selected network is configured to use "
			"SIM, but neither EAP-SIM nor EAP-AKA are enabled");
		return 0;
	}

	wpa_msg(wpa_s, MSG_DEBUG, "Selected network is configured to use SIM "
					 ...
					 struct wpa_scan_res *bss)
{
	...
	while ((ssid->proto & WPA_PROTO_RSN) && rsn_ie) {
		...
		if (wpa_parse_wpa_ie(rsn_ie, 2 + rsn_ie[1], &ie)) {
			wpa_msg(wpa_s, MSG_DEBUG, "   skip RSN IE - parse failed");
			break;
		}
		...
		{
			wpa_msg(wpa_s, MSG_DEBUG, "   selected based on TSN in "
				"RSN IE");
			return 1;
		}

		if (!(ie.proto & ssid->proto)) {
			wpa_msg(wpa_s, MSG_DEBUG, "   skip RSN IE - proto "
				"mismatch");
			break;
		}

		if (!(ie.pairwise_cipher & ssid->pairwise_cipher)) {
			wpa_msg(wpa_s, MSG_DEBUG, "   skip RSN IE - PTK cipher "
				"mismatch");
			break;
		}

		if (!(ie.group_cipher & ssid->group_cipher)) {
			wpa_msg(wpa_s, MSG_DEBUG, "   skip RSN IE - GTK cipher "
				"mismatch");
			break;
		}

		if (!(ie.key_mgmt & ssid->key_mgmt)) {
			wpa_msg(wpa_s, MSG_DEBUG, "   skip RSN IE - key mgmt "
				"mismatch");
			break;
		}
		...
		if (!(ie.capabilities & WPA_CAPABILITY_MFPC) &&
			...
			wpa_msg(wpa_s, MSG_DEBUG, "   skip RSN IE - no mgmt frame "
				   "protection");
			break;
		}
#endif /* CONFIG_IEEE80211W */

		wpa_msg(wpa_s, MSG_DEBUG, "   selected based on RSN IE");
		return 1;
	}
	...
	while ((ssid->proto & WPA_PROTO_WPA) && wpa_ie) {
		...
		if (wpa_parse_wpa_ie(wpa_ie, 2 + wpa_ie[1], &ie)) {
			wpa_msg(wpa_s, MSG_DEBUG, "   skip WPA IE - parse failed");
			break;
		}
		...
		if (!(ie.proto & ssid->proto)) {
			wpa_msg(wpa_s, MSG_DEBUG, "   skip WPA IE - proto "
				"mismatch");
			break;
		}

		if (!(ie.pairwise_cipher & ssid->pairwise_cipher)) {
			wpa_msg(wpa_s, MSG_DEBUG, "   skip WPA IE - PTK cipher "
				"mismatch");
			break;
		}

		if (!(ie.group_cipher & ssid->group_cipher)) {
			wpa_msg(wpa_s, MSG_DEBUG, "   skip WPA IE - GTK cipher "
				"mismatch");
			break;
		}

		if (!(ie.key_mgmt & ssid->key_mgmt)) {
			wpa_msg(wpa_s, MSG_DEBUG, "   skip WPA IE - key mgmt "
				"mismatch");
			break;
		}

		wpa_msg(wpa_s, MSG_DEBUG, "   selected based on WPA IE");
		return 1;
	}

	if ((ssid->proto & (WPA_PROTO_WPA | WPA_PROTO_RSN)) &&
	    wpa_key_mgmt_wpa(ssid->key_mgmt) && proto_match == 0) {
		wpa_msg(wpa_s, MSG_DEBUG, "   skip - no WPA/RSN proto match");
		return 0;
	}
					    ...
					    struct wpa_ssid *group)
{
	...
	wpa_msg(wpa_s, MSG_DEBUG, "%d: " MACSTR " ssid='%s' "
	...
	if (e) {
		...
		if (e->count > limit) {
			wpa_msg(wpa_s, MSG_DEBUG, "   skip - blacklisted "
	...
	if (ssid_len == 0) {
		wpa_msg(wpa_s, MSG_DEBUG, "   skip - SSID not known");
		return 0;
	}
	...
	for (ssid = group; ssid; ssid = ssid->pnext) {
		...
		if (ssid->disabled) {
			wpa_msg(wpa_s, MSG_DEBUG, "   skip - disabled");
			continue;
		}

#ifdef CONFIG_WPS
		if ((ssid->key_mgmt & WPA_KEY_MGMT_WPS) && e && e->count > 0) {
			wpa_msg(wpa_s, MSG_DEBUG, "   skip - blacklisted (WPS)");
			continue;
		}
		...
		if (check_ssid &&
			...
			wpa_msg(wpa_s, MSG_DEBUG, "   skip - SSID mismatch");
			continue;
		}

		if (ssid->bssid_set &&
		    os_memcmp(bss->bssid, ssid->bssid, ETH_ALEN) != 0) {
			wpa_msg(wpa_s, MSG_DEBUG, "   skip - BSSID mismatch");
			continue;
		}
		...
		if (!wpa &&
		    ...
		    !(ssid->key_mgmt & WPA_KEY_MGMT_IEEE8021X_NO_WPA)) {
			wpa_msg(wpa_s, MSG_DEBUG, "   skip - non-WPA network not "
				"allowed");
			continue;
		}

		if (!wpa && !wpa_supplicant_match_privacy(bss, ssid)) {
			wpa_msg(wpa_s, MSG_DEBUG, "   skip - privacy mismatch");
			continue;
		}

		if (!wpa && (bss->caps & IEEE80211_CAP_IBSS)) {
			wpa_msg(wpa_s, MSG_DEBUG, "   skip - IBSS (adhoc) "
				"network");
			continue;
		}

		if (!freq_allowed(ssid->freq_list, bss->freq)) {
			wpa_msg(wpa_s, MSG_DEBUG, "   skip - frequency not "
				"allowed");
			continue;
		}
			  ...
			  struct wpa_ssid **selected_ssid)
{
	...
	wpa_msg(wpa_s, MSG_DEBUG, "Selecting BSS from priority group %d",
		group->priority);
	...
	for (i = 0; i < scan_res->num; i++) {
		...
		*selected_ssid = wpa_scan_res_match(wpa_s, i, bss, group);
		...
		wpa_msg(wpa_s, MSG_DEBUG, "   selected BSS " MACSTR " ssid='%s'",
			    ...
			    struct wpa_ssid **selected_ssid)
{
	...
	while (selected == NULL) {
		...
		if (selected == NULL && wpa_s->blacklist) {
			wpa_msg(wpa_s, MSG_DEBUG, "No APs found - clear blacklist "
				"and try again");
				       ...
				       struct wpa_scan_results *scan_res)
{
	...
	wpa_msg(wpa_s, MSG_DEBUG, "Considering within-ESS reassociation");
	wpa_msg(wpa_s, MSG_DEBUG, "Current BSS: " MACSTR " level=%d",
		MAC2STR(current_bss->bssid), current_bss->level);
	wpa_msg(wpa_s, MSG_DEBUG, "Selected BSS: " MACSTR " level=%d",
		MAC2STR(selected->bssid), selected->level);

	if (wpa_s->current_ssid->bssid_set &&
	    os_memcmp(selected->bssid, wpa_s->current_ssid->bssid, ETH_ALEN) ==
	    0) {
		wpa_msg(wpa_s, MSG_DEBUG, "Allow reassociation - selected BSS has "
			"preferred BSSID");
		return 1;
	}
	...
	if (abs(current_bss->level - selected->level) < min_diff) {
		wpa_msg(wpa_s, MSG_DEBUG, "Skip roam - too small difference in "
			"signal level");
		return 0;
	}
					       ...
					       union wpa_event_data *data)
{
	...
	if (selected) {
		...
		wpa_scan_results_free(scan_res);
		wpa_msg(wpa_s, MSG_DEBUG, "No suitable network found");
		ssid = wpa_supplicant_pick_new_network(wpa_s);
		if (ssid) {
			wpa_msg(wpa_s, MSG_DEBUG, "Setup a new network");
					      ...
					      union wpa_event_data *data)
{
	...
	wpa_msg(wpa_s, MSG_DEBUG, "%s: Checking for other virtual interfaces "
	...
	for (ifs = wpa_s->global->ifaces; ifs; ifs = ifs->next) {
		...
		if (rn2 && os_strcmp(rn, rn2) == 0) {
			wpa_msg(wpa_s, MSG_DEBUG, "%s: Updating scan results from "
				"sibling", ifs->ifname);
					  ...
					  union wpa_event_data *data)
{
	...
	wpa_msg(wpa_s, MSG_DEBUG, "Association info event");
	...
	if (data->assoc_info.freq)
		wpa_msg(wpa_s, MSG_DEBUG, "freq=%u MHz", data->assoc_info.freq);
	...
	if (wpa_s->sme.auth_alg == WPA_AUTH_ALG_FT) {
		...
		if (wpa_drv_get_bssid(wpa_s, bssid) < 0 ||
		    wpa_ft_validate_reassoc_resp(wpa_s->wpa,
						 ...
						 data->assoc_info.resp_ies_len,
						 bssid) < 0) {
			wpa_msg(wpa_s, MSG_DEBUG, "FT: Validation of "
	...
	if (wpa_s->current_ssid &&
		...
		if (wps == NULL) {
			wpa_msg(wpa_s, MSG_INFO, "WPS-STRICT: AP did not include "
				       ...
				       union wpa_event_data *data)
{
	...
	if (wpa_s->pending_eapol_rx) {
		...
		if (age.sec == 0 && age.usec < 100000 &&
		    ...
		    0) {
			wpa_msg(wpa_s, MSG_DEBUG, "Process pending EAPOL frame "
	...
	if (wpa_s->current_ssid != wpa_s->bgscan_ssid) {
		...
		if (wpa_s->current_ssid && wpa_s->current_ssid->bgscan) {
			if (bgscan_init(wpa_s, wpa_s->current_ssid)) {
				wpa_msg(wpa_s, MSG_DEBUG, "Failed to initialize "
					"bgscan");
				/*
					  ...
					  u16 reason_code)
{
	...
	if (wpa_s->key_mgmt == WPA_KEY_MGMT_WPA_NONE) {
		 ...
		 */
		wpa_msg(wpa_s, MSG_DEBUG, "Disconnect event - ignore in "
			"IBSS/WPA-None mode");
		return;
	}
	...
	if (!wpa_s->auto_reconnect_disabled ||
		...
		wpa_msg(wpa_s, MSG_DEBUG, "WPA: Auto connect enabled: try to "
			"reconnect (wps=%d)",
			wpa_s->key_mgmt == WPA_KEY_MGMT_WPS);
		if (wpa_s->wpa_state >= WPA_ASSOCIATING)
			wpa_supplicant_req_scan(wpa_s, 0, 100000);
	} else {
		wpa_msg(wpa_s, MSG_DEBUG, "WPA: Auto connect disabled: do not try "
			"to re-connect");
		wpa_s->reassociate = 0;
	...
	if (authenticating &&
		 ...
		 */
		wpa_msg(wpa_s, MSG_DEBUG, "SME: Allow pending authentication to "
						    ...
						    void *sock_ctx)
{
	...
	wpa_msg(wpa_s, MSG_DEBUG, "WPA: Sending pending MIC error report");
					 ...
					 union wpa_event_data *data)
{
	...
	if ((wpa_s->last_michael_mic_error &&
		...
		if (wpa_s->mic_errors_seen) {
			...
			wpa_msg(wpa_s, MSG_DEBUG, "WPA: Delay MIC error report %d "
				"seconds", sec);
				      ...
				      union wpa_event_data *data)
{
	...
	case EVENT_INTERFACE_ADDED:
		...
		wpa_msg(wpa_s, MSG_DEBUG, "Configured interface was added.");
		if (wpa_supplicant_driver_init(wpa_s) < 0) {
			wpa_msg(wpa_s, MSG_INFO, "Failed to initialize the driver "
				"after interface was added.");
		}
		break;
	case EVENT_INTERFACE_REMOVED:
		wpa_msg(wpa_s, MSG_DEBUG, "Configured interface was removed.");
			 ...
			 size_t len)
{
	...
	wpa_msg(wpa_s, MSG_DEBUG, "FT: Received FT Action Response: STA " MACSTR
		" TargetAP " MACSTR " status %u",
		MAC2STR(sta_addr), MAC2STR(target_ap_addr), status);

	if (os_memcmp(sta_addr, wpa_s->own_addr, ETH_ALEN) != 0) {
		wpa_msg(wpa_s, MSG_DEBUG, "FT: Foreign STA Address " MACSTR
			" in FT Action Response", MAC2STR(sta_addr));
		return;
	}

	if (status) {
		wpa_msg(wpa_s, MSG_DEBUG, "FT: FT Action Response indicates "
			  ...
			  union wpa_event_data *data)
{
	...
	if (wpa_s->wpa_state == WPA_INTERFACE_DISABLED &&
		...
		wpa_msg(wpa_s, MSG_DEBUG, "Ignore event %d while interface is "
			"disabled", event);
		return;
	}

	wpa_msg(wpa_s, MSG_DEBUG, "Event %d received on interface %s",
		event, wpa_s->ifname);
	...
	case EVENT_ASSOC:
		...
		break;
	case EVENT_DISASSOC:
		wpa_msg(wpa_s, MSG_DEBUG, "Disassociation notification");
		if (data) {
			wpa_msg(wpa_s, MSG_DEBUG, " * reason %u",
				data->disassoc_info.reason_code);
			if (data->disassoc_info.addr)
				wpa_msg(wpa_s, MSG_DEBUG, " * address " MACSTR,
	...
	case EVENT_DEAUTH:
		if (event == EVENT_DEAUTH) {
			wpa_msg(wpa_s, MSG_DEBUG, "Deauthentication notification");
			if (data) {
				reason_code = data->deauth_info.reason_code;
				wpa_msg(wpa_s, MSG_DEBUG, " * reason %u",
					data->deauth_info.reason_code);
				if (data->deauth_info.addr) {
					wpa_msg(wpa_s, MSG_DEBUG, " * address "
						MACSTR,
	...
	case EVENT_TX_STATUS:
		wpa_msg(wpa_s, MSG_DEBUG, "EVENT_TX_STATUS on %s dst=" MACSTR
			" type=%d stype=%d",
...
#ifdef CONFIG_P2P
		wpa_msg(wpa_s, MSG_DEBUG, "EVENT_TX_STATUS pending_dst=" MACSTR,
...
#endif /* CONFIG_P2P */
			wpa_msg(wpa_s, MSG_DEBUG, "AP: ignore received management "
	...
	case EVENT_RX_ACTION:
		wpa_msg(wpa_s, MSG_DEBUG, "Received Action frame: SA=" MACSTR
	...
	case EVENT_INTERFACE_ENABLED:
		wpa_msg(wpa_s, MSG_DEBUG, "Interface was enabled");
		if (wpa_s->wpa_state == WPA_INTERFACE_DISABLED) {
#ifdef CONFIG_AP
			if (!wpa_s->ap_iface) {
	...
	case EVENT_INTERFACE_DISABLED:
		wpa_msg(wpa_s, MSG_DEBUG, "Interface was disabled");
		wpa_supplicant_mark_disassoc(wpa_s);
	...
	case EVENT_BEST_CHANNEL:
		wpa_msg(wpa_s, MSG_DEBUG, "Best channel event received (%d %d %d)",
