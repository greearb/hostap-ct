			       ...
			       const char *buf, char *pos, int line)
{
	if (os_strcmp(buf, "interface") == 0) {
	...
	} else if (os_strcmp(buf, "radius_server_dev") == 0) {
			os_free(bss->radius_server_dev);
			bss->radius_server_dev = os_strdup(pos);
		...
		wpa_printf(MSG_ERROR, "Line %d: set he_bss_color: %d",
			   line, conf->he_op.he_bss_color);
...
#define PARSE_TEST_PROBABILITY(_val)				\
	...
	PARSE_TEST_PROBABILITY(dup_eapol_1_of_4)
	PARSE_TEST_PROBABILITY(dup_eapol_3_of_4)
