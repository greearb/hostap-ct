						 ...
						 u8 *buf, size_t len)
{
	...
	wpa_dbg(drv->ctx, MSG_DEBUG, "RTM_NEWLINK: operstate=%d ifi_flags=0x%x "
		"(%s%s%s%s)",
	...
	if (!drv->if_disabled && !(ifi->ifi_flags & IFF_UP)) {
		...
		wpa_dbg(drv->ctx, MSG_DEBUG, "nl80211: Interface down");
	...
	if (drv->if_disabled && (ifi->ifi_flags & IFF_UP)) {
		wpa_dbg(drv->ctx, MSG_DEBUG, "nl80211: Interface up");
		drv->if_disabled = 0;
