			...
			u8 *msg, size_t msg_len, u8 *key_mic)
{
	if (is_zero_ether_addr(dest) && is_zero_ether_addr(sm->bssid)) {
		...
		if (wpa_sm_get_bssid(sm, sm->bssid) < 0) {
			wpa_msg(sm->ctx->msg_ctx, MSG_DEBUG, "WPA: Failed to read BSSID for "
				"EAPOL-Key destination address");
		} else {
			dest = sm->bssid;
			wpa_msg(sm->ctx->msg_ctx, MSG_DEBUG, "WPA: Use BSSID (" MACSTR
	...
	if (key_mic &&
		...
		wpa_msg(sm->ctx->msg_ctx, MSG_ERROR, "WPA: Failed to generate EAPOL-Key "
			"version %d MIC", ver);
		goto out;
	}
...
void wpa_sm_key_request(struct wpa_sm *sm, int error, int pairwise)
{
	...
	if (wpa_sm_get_bssid(sm, bssid) < 0) {
		wpa_msg(sm->ctx->msg_ctx, MSG_WARNING, "Failed to read BSSID for EAPOL-Key "
			"request");
		return;
	}
	...
	wpa_msg(sm->ctx->msg_ctx, MSG_INFO, "WPA: Sending EAPOL-Key Request (error=%d "
				  ...
				  const u8 *pmkid)
{
	...
	if (pmkid && !sm->cur_pmksa) {
		...
		if (sm->cur_pmksa) {
			wpa_msg(sm->ctx->msg_ctx, MSG_DEBUG, "RSN: found matching PMKID from "
				"PMKSA cache");
		} else {
			wpa_msg(sm->ctx->msg_ctx, MSG_DEBUG, "RSN: no matching PMKID found");
			abort_cached = 1;
		}
	}
	...
	if (pmkid && sm->cur_pmksa &&
		...
		if (res == 0) {
			...
			if (!sm->cur_pmksa && pmkid &&
				...
				wpa_msg(sm->ctx->msg_ctx, MSG_DEBUG, "RSN: the new PMK "
			...
			if (sm->cur_pmksa) {
				wpa_msg(sm->ctx->msg_ctx, MSG_DEBUG, "RSN: Cancelled PMKSA "
					"caching attempt");
				sm->cur_pmksa = NULL;
				abort_cached = 1;
	...
	if (abort_cached && wpa_key_mgmt_wpa_ieee8021x(sm->key_mgmt)) {
		...
		wpa_msg(sm->ctx->msg_ctx, MSG_DEBUG, "RSN: no PMKSA entry found - trigger "
			       ...
			       struct wpa_ptk *ptk)
{
	...
	if (wpa_ie == NULL) {
		wpa_msg(sm->ctx->msg_ctx, MSG_WARNING, "WPA: No wpa_ie set - cannot "
			"generate msg 2/4");
		return -1;
	}
	...
	wpa_msg(sm->ctx->msg_ctx, MSG_DEBUG, "WPA: Sending EAPOL-Key 2/4");
					  ...
					  u16 ver)
{
	...
	if (wpa_sm_get_network_ctx(sm) == NULL) {
		wpa_msg(sm->ctx->msg_ctx, MSG_WARNING, "WPA: No SSID info found (msg 1 of "
			"4).");
		return;
	}

	wpa_sm_set_state(sm, WPA_4WAY_HANDSHAKE);
	wpa_msg(sm->ctx->msg_ctx, MSG_DEBUG, "WPA: RX message 1 of 4-Way Handshake from "
	...
	if (res == -2) {
		wpa_msg(sm->ctx->msg_ctx, MSG_DEBUG, "RSN: Do not reply to msg 1/4 - "
					    ...
					    const u8 *addr, int secure)
{
	...
	if (sm->cur_pmksa && sm->cur_pmksa->opportunistic) {
		wpa_msg(sm->ctx->msg_ctx, MSG_DEBUG, "RSN: Authenticator accepted "
...
{
	struct wpa_sm *sm = eloop_ctx;
	wpa_msg(sm->ctx->msg_ctx, MSG_DEBUG, "WPA: Request PTK rekeying");
				      ...
				      const struct wpa_eapol_key *key)
{
	...
	wpa_msg(sm->ctx->msg_ctx, MSG_DEBUG, "WPA: Installing PTK to the driver.");
	...
	case WPA_CIPHER_NONE:
		wpa_msg(sm->ctx->msg_ctx, MSG_DEBUG, "WPA: Pairwise Cipher Suite: "
			"NONE - do not use pairwise keys");
		return 0;
	default:
		wpa_msg(sm->ctx->msg_ctx, MSG_WARNING, "WPA: Unsupported pairwise cipher %d",
			   sm->pairwise_cipher);
		return -1;
	}
	...
	if (wpa_sm_set_key(sm, alg, sm->bssid, 0, 1, key_rsc, rsclen,
		...
		wpa_msg(sm->ctx->msg_ctx, MSG_WARNING, "WPA: Failed to set PTK to the "
					     ...
					     enum wpa_alg *alg)
{
	...
	if (ret < 0 ) {
		wpa_msg(sm->ctx->msg_ctx, MSG_WARNING, "WPA: Unsupported %s Group Cipher key "
			"length %d (%d).",
				      ...
				      const u8 *key_rsc)
{
	...
	wpa_msg(sm->ctx->msg_ctx, MSG_DEBUG, "WPA: Installing GTK to the driver "
	...
	if (sm->pairwise_cipher == WPA_CIPHER_NONE) {
		if (wpa_sm_set_key(sm, gd->alg, NULL,
				   ...
				   _gtk, gd->gtk_len) < 0) {
			wpa_msg(sm->ctx->msg_ctx, MSG_WARNING, "WPA: Failed to set "
			...
			return -1;
				  ...
				  _gtk, gd->gtk_len) < 0) {
		wpa_msg(sm->ctx->msg_ctx, MSG_WARNING, "WPA: Failed to set GTK to "
						...
						int tx)
{
	if (tx && sm->pairwise_cipher != WPA_CIPHER_NONE) {
		...
		wpa_msg(sm->ctx->msg_ctx, MSG_INFO, "WPA: Tx bit set for GTK, but pairwise "
				       ...
				       int key_info)
{
	...
	if (wpa_supplicant_check_group_cipher(sm, sm->group_cipher,
		...
		wpa_msg(sm->ctx->msg_ctx, MSG_DEBUG, "RSN: Failed to install GTK");
		return -1;
	}
			       ...
			       struct wpa_eapol_ie_parse *ie)
{
	...
	if (ie->igtk) {
		...
		wpa_msg(sm->ctx->msg_ctx, MSG_DEBUG, "WPA: IGTK keyid %d "
		...
		if (wpa_sm_set_key(sm, WPA_ALG_IGTK, broadcast_ether_addr,
			...
			wpa_msg(sm->ctx->msg_ctx, MSG_WARNING, "WPA: Failed to configure IGTK"
				" to the driver");
			return -1;
		}
	}
				   ...
				   const u8 *rsn_ie, size_t rsn_ie_len)
{
	...
	if (wpa_ie) {
		if (!sm->ap_wpa_ie) {
			wpa_msg(sm->ctx->msg_ctx, MSG_INFO, "WPA: No WPA IE in "
				"Beacon/ProbeResp");
		}
	...
	if (rsn_ie) {
		if (!sm->ap_rsn_ie) {
			wpa_msg(sm->ctx->msg_ctx, MSG_INFO, "WPA: No RSN IE in "
				"Beacon/ProbeResp");
		}
			    ...
			    const u8 *assoc_resp_mdie)
{
	...
	if (ie->mdie == NULL || ie->mdie_len < 2 + sizeof(*mdie) ||
		...
		wpa_msg(sm->ctx->msg_ctx, MSG_DEBUG, "FT: MDIE in msg 3/4 did not "
	...
	if (assoc_resp_mdie &&
		...
		wpa_msg(sm->ctx->msg_ctx, MSG_DEBUG, "FT: MDIE mismatch");
...
{
	if (ie->ftie == NULL) {
		wpa_msg(sm->ctx->msg_ctx, MSG_DEBUG, "FT: No FTIE in EAPOL-Key msg 3/4");
		return -1;
	}
	...
	if (assoc_resp_ftie[1] != ie->ftie[1] ||
		...
		wpa_msg(sm->ctx->msg_ctx, MSG_DEBUG, "FT: FTIE mismatch");
			     ...
			     struct wpa_eapol_ie_parse *ie)
{
	...
	if (wpa_parse_wpa_ie_rsn(ie->rsn_ie, ie->rsn_ie_len, &rsn) < 0 ||
		...
		wpa_msg(sm->ctx->msg_ctx, MSG_DEBUG, "FT: No PMKR1Name in "
			"FT 4-way handshake message 3/4");
		return -1;
	}

	if (os_memcmp(rsn.pmkid, sm->pmk_r1_name, WPA_PMK_NAME_LEN) != 0) {
		wpa_msg(sm->ctx->msg_ctx, MSG_DEBUG, "FT: PMKR1Name mismatch in "
			"FT 4-way handshake message 3/4");
				      ...
				      struct wpa_eapol_ie_parse *ie)
{
	if (sm->ap_wpa_ie == NULL && sm->ap_rsn_ie == NULL) {
		wpa_msg(sm->ctx->msg_ctx, MSG_DEBUG, "WPA: No WPA/RSN IE for this AP known. "
			"Trying to get from scan results");
		if (wpa_sm_get_beacon_ie(sm) < 0) {
			wpa_msg(sm->ctx->msg_ctx, MSG_WARNING, "WPA: Could not find AP from "
				"the scan results");
		} else {
			wpa_msg(sm->ctx->msg_ctx, MSG_DEBUG, "WPA: Found the current AP from "
			       ...
			       struct wpa_ptk *ptk)
{
	...
	wpa_msg(sm->ctx->msg_ctx, MSG_DEBUG, "WPA: Sending EAPOL-Key 4/4");
	wpa_eapol_key_send(sm, ptk->kck, ver, dst, ETH_P_EAPOL,
			   rbuf, rlen, reply->key_mic);
					  ...
					  u16 ver)
{
	...
	wpa_sm_set_state(sm, WPA_4WAY_HANDSHAKE);
	wpa_msg(sm->ctx->msg_ctx, MSG_DEBUG, "WPA: RX message 3 of 4-Way Handshake from "
	...
	if (ie.igtk && !(key_info & WPA_KEY_INFO_ENCR_KEY_DATA)) {
		wpa_msg(sm->ctx->msg_ctx, MSG_WARNING, "WPA: IGTK KDE in unencrypted key "
			"data");
		goto failed;
	}
	...
	if (os_memcmp(sm->anonce, key->key_nonce, WPA_NONCE_LEN) != 0) {
		wpa_msg(sm->ctx->msg_ctx, MSG_WARNING, "WPA: ANonce from message 1 of 4-Way "
	...
	case WPA_CIPHER_CCMP:
		if (keylen != 16) {
			wpa_msg(sm->ctx->msg_ctx, MSG_WARNING, "WPA: Invalid CCMP key length "
				"%d (src=" MACSTR ")",
	...
	case WPA_CIPHER_TKIP:
		if (keylen != 32) {
			wpa_msg(sm->ctx->msg_ctx, MSG_WARNING, "WPA: Invalid TKIP key length "
				"%d (src=" MACSTR ")",
				keylen, MAC2STR(sm->bssid));
			goto failed;
		}
					     ...
					     struct wpa_gtk_data *gd)
{
	...
	if (ie.gtk_len - 2 > sizeof(gd->gtk)) {
		wpa_msg(sm->ctx->msg_ctx, MSG_INFO, "RSN: Too long GTK in GTK IE "
					     ...
					     struct wpa_gtk_data *gd)
{
	...
	if (keydatalen > extra_len) {
		wpa_msg(sm->ctx->msg_ctx, MSG_INFO, "WPA: Truncated EAPOL-Key packet:"
	...
	if (ver == WPA_KEY_INFO_TYPE_HMAC_MD5_RC4) {
		...
		if (keydatalen > sizeof(gd->gtk)) {
			wpa_msg(sm->ctx->msg_ctx, MSG_WARNING, "WPA: RC4 key data "
				"too long (%lu)",
				(unsigned long) keydatalen);
			return -1;
		}
		...
		if (keydatalen % 8) {
			wpa_msg(sm->ctx->msg_ctx, MSG_WARNING, "WPA: Unsupported AES-WRAP "
				"len %lu", (unsigned long) keydatalen);
			return -1;
		}
		if (maxkeylen > sizeof(gd->gtk)) {
			wpa_msg(sm->ctx->msg_ctx, MSG_WARNING, "WPA: AES-WRAP key data "
		...
		if (aes_unwrap(sm->ptk.kek, maxkeylen / 8,
			...
			wpa_msg(sm->ctx->msg_ctx, MSG_WARNING, "WPA: AES unwrap "
				      ...
				      int ver, u16 key_info)
{
	...
	wpa_msg(sm->ctx->msg_ctx, MSG_DEBUG, "WPA: Sending EAPOL-Key 2/2");
					  ...
					  int extra_len, u16 ver)
{
	...
	wpa_msg(sm->ctx->msg_ctx, MSG_DEBUG, "WPA: RX message 1 of Group Key Handshake from "
		MACSTR " (ver=%d)", MAC2STR(src_addr), ver);
					       ...
					       const u8 *buf, size_t len)
{
	...
	if (sm->tptk_set) {
		...
		if (os_memcmp(mic, key->key_mic, 16) != 0) {
			wpa_msg(sm->ctx->msg_ctx, MSG_WARNING, "WPA: Invalid EAPOL-Key MIC "
	...
	if (!ok) {
		wpa_msg(sm->ctx->msg_ctx, MSG_WARNING, "WPA: Could not verify EAPOL-Key MIC "
			"- dropping packet");
		return -1;
	}
					   ...
					   struct wpa_eapol_key *key, u16 ver)
{
	...
	wpa_hexdump(MSG_DEBUG, "RSN: encrypted key data",
		    (u8 *) (key + 1), keydatalen);
	if (!sm->ptk_set) {
		wpa_msg(sm->ctx->msg_ctx, MSG_WARNING, "WPA: PTK not available, "
	...
	if (ver == WPA_KEY_INFO_TYPE_HMAC_MD5_RC4) {
		...
		u8 *buf;
		if (keydatalen % 8) {
			wpa_msg(sm->ctx->msg_ctx, MSG_WARNING, "WPA: Unsupported "
		...
		if (buf == NULL) {
			wpa_msg(sm->ctx->msg_ctx, MSG_WARNING, "WPA: No memory for "
				"AES-UNWRAP buffer");
			return -1;
		}
		if (aes_unwrap(sm->ptk.kek, keydatalen / 8,
			       (u8 *) (key + 1), buf)) {
			os_free(buf);
			wpa_msg(sm->ctx->msg_ctx, MSG_WARNING, "WPA: AES unwrap failed - "
...
{
	if (sm && sm->cur_pmksa) {
		wpa_msg(sm->ctx->msg_ctx, MSG_DEBUG, "RSN: Cancelling PMKSA caching attempt");
		sm->cur_pmksa = NULL;
	}
}
			       ...
			       const struct wpa_eapol_key *key)
{
	...
	wpa_msg(sm->ctx->msg_ctx, MSG_DEBUG, "  EAPOL-Key type=%d", key->type);
	wpa_msg(sm->ctx->msg_ctx, MSG_DEBUG, "  key_info 0x%x (ver=%d keyidx=%d rsvd=%d %s"
		"%s%s%s%s%s%s%s)",
	...
	wpa_msg(sm->ctx->msg_ctx, MSG_DEBUG, "  key_length=%u key_data_length=%u",
		    ...
		    const u8 *buf, size_t len)
{
	...
	if (len < sizeof(*hdr) + sizeof(*key)) {
		wpa_msg(sm->ctx->msg_ctx, MSG_DEBUG, "WPA: EAPOL frame too short to be a WPA "
	...
	wpa_msg(sm->ctx->msg_ctx, MSG_DEBUG, "IEEE 802.1X RX: version=%d type=%d length=%lu",
	...
	if (hdr->type != IEEE802_1X_TYPE_EAPOL_KEY) {
		wpa_msg(sm->ctx->msg_ctx, MSG_DEBUG, "WPA: EAPOL frame (type %u) discarded, "
			"not a Key frame", hdr->type);
		ret = 0;
		goto out;
	}
	if (plen > len - sizeof(*hdr) || plen < sizeof(*key)) {
		wpa_msg(sm->ctx->msg_ctx, MSG_DEBUG, "WPA: EAPOL frame payload size %lu "
	...
	{
		wpa_msg(sm->ctx->msg_ctx, MSG_DEBUG, "WPA: EAPOL-Key type (%d) unknown, "
			"discarded", key->type);
		ret = 0;
		goto out;
	}
	...
	if (data_len < len) {
		wpa_msg(sm->ctx->msg_ctx, MSG_DEBUG, "WPA: ignoring %lu bytes after the IEEE "
	...
	if (ver != WPA_KEY_INFO_TYPE_HMAC_MD5_RC4 &&
	    ...
	    ver != WPA_KEY_INFO_TYPE_HMAC_SHA1_AES) {
		wpa_msg(sm->ctx->msg_ctx, MSG_INFO, "WPA: Unsupported EAPOL-Key descriptor "
			"version %d.", ver);
		goto out;
	}
	...
	if (wpa_key_mgmt_ft(sm->key_mgmt)) {
		...
		if (ver != WPA_KEY_INFO_TYPE_AES_128_CMAC) {
			wpa_msg(sm->ctx->msg_ctx, MSG_INFO, "FT: AP did not use "
				"AES-128-CMAC.");
			goto out;
		}
	} else
	...
	if (wpa_key_mgmt_sha256(sm->key_mgmt)) {
		if (ver != WPA_KEY_INFO_TYPE_AES_128_CMAC) {
			wpa_msg(sm->ctx->msg_ctx, MSG_INFO, "WPA: AP did not use the "
	...
	if (sm->pairwise_cipher == WPA_CIPHER_CCMP &&
	    ver != WPA_KEY_INFO_TYPE_HMAC_SHA1_AES) {
		wpa_msg(sm->ctx->msg_ctx, MSG_INFO, "WPA: CCMP is used, but EAPOL-Key "
		...
		if (sm->group_cipher != WPA_CIPHER_CCMP &&
			...
			wpa_msg(sm->ctx->msg_ctx, MSG_INFO, "WPA: Backwards compatibility: "
	...
	if (!(key_info & WPA_KEY_INFO_SMK_MESSAGE) && peerkey) {
		if (!peerkey->initiator && peerkey->replay_counter_set &&
			...
			wpa_msg(sm->ctx->msg_ctx, MSG_WARNING, "RSN: EAPOL-Key Replay "
			...
			if (os_memcmp(_tmp, peerkey->replay_counter,
				      WPA_REPLAY_COUNTER_LEN) != 0) {
				wpa_msg(sm->ctx->msg_ctx, MSG_DEBUG, "RSN: EAPOL-Key Replay "
	...
	if (!peerkey && sm->rx_replay_counter_set &&
	    os_memcmp(key->replay_counter, sm->rx_replay_counter,
		      WPA_REPLAY_COUNTER_LEN) <= 0) {
		wpa_msg(sm->ctx->msg_ctx, MSG_WARNING, "WPA: EAPOL-Key Replay Counter did not"
	...
	if (key_info & WPA_KEY_INFO_REQUEST) {
		wpa_msg(sm->ctx->msg_ctx, MSG_INFO, "WPA: EAPOL-Key with Request bit - "
			"dropped");
		goto out;
	}
	...
	if (key_info & WPA_KEY_INFO_KEY_TYPE) {
		if (key_info & WPA_KEY_INFO_KEY_INDEX_MASK) {
			wpa_msg(sm->ctx->msg_ctx, MSG_WARNING, "WPA: Ignored EAPOL-Key "
		...
		if (key_info & WPA_KEY_INFO_MIC) {
			...
			wpa_msg(sm->ctx->msg_ctx, MSG_WARNING, "WPA: EAPOL-Key (Group) "
				 ...
				 void *ctx, int replace)
{
	...
	if (sm->cur_pmksa == entry ||
		...
		wpa_msg(sm->ctx->msg_ctx, MSG_DEBUG, "RSN: removed current PMKSA entry");
		sm->cur_pmksa = NULL;
...
struct wpa_sm * wpa_sm_init(struct wpa_sm_ctx *ctx)
{
	...
	if (sm->pmksa == NULL) {
		wpa_msg(sm->ctx->msg_ctx, MSG_ERROR, "RSN: PMKSA cache initialization "
			"failed");
		os_free(sm);
		return NULL;
	}
...
void wpa_sm_notify_assoc(struct wpa_sm *sm, const u8 *bssid)
{
	...
	wpa_msg(sm->ctx->msg_ctx, MSG_DEBUG, "WPA: Association event - clear replay counter");
	...
	if (clear_ptk) {
		 ...
		 */
		wpa_msg(sm->ctx->msg_ctx, MSG_DEBUG, "WPA: Clear old PTK");
		sm->ptk_set = 0;
		sm->tptk_set = 0;
	}
}
...
int wpa_sm_set_assoc_wpa_ie(struct wpa_sm *sm, const u8 *ie, size_t len)
{
	...
	os_free(sm->assoc_wpa_ie);
	if (ie == NULL || len == 0) {
		wpa_msg(sm->ctx->msg_ctx, MSG_DEBUG, "WPA: clearing own WPA/RSN IE");
...
int wpa_sm_set_ap_wpa_ie(struct wpa_sm *sm, const u8 *ie, size_t len)
{
	...
	os_free(sm->ap_wpa_ie);
	if (ie == NULL || len == 0) {
		wpa_msg(sm->ctx->msg_ctx, MSG_DEBUG, "WPA: clearing AP WPA IE");
		sm->ap_wpa_ie = NULL;
		sm->ap_wpa_ie_len = 0;
	} else {
...
int wpa_sm_set_ap_rsn_ie(struct wpa_sm *sm, const u8 *ie, size_t len)
{
	...
	os_free(sm->ap_rsn_ie);
	if (ie == NULL || len == 0) {
		wpa_msg(sm->ctx->msg_ctx, MSG_DEBUG, "WPA: clearing AP RSN IE");
		sm->ap_rsn_ie = NULL;
		sm->ap_rsn_ie_len = 0;
	} else {
...
int wpa_sm_parse_own_wpa_ie(struct wpa_sm *sm, struct wpa_ie_data *data)
{
	if (sm == NULL || sm->assoc_wpa_ie == NULL) {
		wpa_msg(sm->ctx->msg_ctx, MSG_DEBUG, "WPA: No WPA/RSN IE available from "
			"association info");
		return -1;
	}
...
{
	wpa_msg(sm->ctx->msg_ctx, MSG_DEBUG, "WPA: Clear old PMK and PTK");
	sm->ptk_set = 0;
	sm->tptk_set = 0;
	os_memset(sm->pmk, 0, sizeof(sm->pmk));
