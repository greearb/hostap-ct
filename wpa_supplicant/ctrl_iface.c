	...
	struct wpa_supplicant *wpa_s, char *cmd)
{
	...
	if (!was_disabled && wpa_s->sched_scanning) {
		wpa_dbg(wpa_s, MSG_DEBUG, "Stop ongoing sched_scan to remove "
	...
	struct wpa_supplicant *wpa_s, char *cmd, char *buf, size_t buflen)
{
	...
	*name++ = '\0';
	...
	id = atoi(cmd);
	wpa_dbg(wpa_s, MSG_DEBUG, "CTRL_IFACE: GET_NETWORK id=%d name='%s'",
		   id, name);
	...
	struct wpa_supplicant *wpa_s, char *cmd)
{
	...
	*name++ = '\0';
	...
	id_s = atoi(cmd);
	id_d = atoi(id);
	wpa_dbg(wpa_s, MSG_DEBUG, "CTRL_IFACE: DUP_NETWORK id=%d -> %d name='%s'",
		   id_s, id_d, name);
...
static int p2p_ctrl_group_add(struct wpa_supplicant *wpa_s, char *cmd)
{
	...
	wpa_dbg(wpa_s, MSG_DEBUG, "CTRL: Invalid P2P_GROUP_ADD parameters '%s'",
		   cmd);
	return -1;
}
...
static int wpas_ctrl_iface_wnm_bss_query(struct wpa_supplicant *wpa_s, char *cmd)
{
	...
	wpa_dbg(wpa_s, MSG_DEBUG, "CTRL_IFACE: WNM_BSS_QUERY query_reason=%d",
		   query_reason);
...
static void wpas_ctrl_vendor_elem_update(struct wpa_supplicant *wpa_s)
{
	...
	wpa_dbg(wpa_s, MSG_DEBUG, "Update vendor elements");
