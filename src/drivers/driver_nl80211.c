						 ...
						 u8 *buf, size_t len)
{
	...
	if (!drv) {
		wpa_msg(NULL, MSG_DEBUG, "nl80211: Ignore dellink event for"
			     ...
			     struct nlattr *tb[])
{
	...
	case NL80211_CMD_REG_CHANGE:
		wpa_msg(drv->ctx, MSG_DEBUG, "nl80211: Regulatory domain change");
...
static int wpa_driver_nl80211_capa(struct wpa_driver_nl80211_data *drv)
{
	...
	if (info.auth_supported)
		...
		wpa_msg(drv->ctx, MSG_INFO, "nl80211: Driver does not support "
...
static void wpa_driver_nl80211_deinit(void *priv)
{
	...
	if (drv->added_if_into_bridge) {
		if (linux_br_del_if(drv->global->ioctl_sock, drv->brname,
				    bss->ifname) < 0)
	...
	if (drv->added_bridge) {
		if (linux_br_del(drv->global->ioctl_sock, drv->brname) < 0)
			wpa_msg(drv->ctx, MSG_INFO, "nl80211: Failed to remove "
				"bridge %s: %s",
				  ...
				  u16 type, const u8 *match, size_t match_len)
{
	...
	genlmsg_put(msg, 0, 0, genl_family_get_id(drv->global->nl80211), 0, 0,
...
wpa_driver_nl80211_finish_drv_init(struct wpa_driver_nl80211_data *drv)
{
	...
	if (wpa_driver_nl80211_set_mode(bss, IEEE80211_MODE_INFRA) < 0) {
		wpa_msg(drv->ctx, MSG_DEBUG, "nl80211: Could not configure driver to "
			"use managed mode");
	}
				   ...
				   struct wpa_driver_scan_params *params)
{
	...
	msg = NULL;
	if (ret) {
		wpa_msg(drv->ctx, MSG_DEBUG, "nl80211: Scan trigger failed: ret=%d "
			"(%s)", ret, strerror(-ret));
#ifdef HOSTAPD
		if (drv->nlmode == NL80211_IFTYPE_AP) {
			/*
	...
	for (i = 0; i < res->num; i++) {
		...
		if (r->flags & WPA_SCAN_AUTHENTICATED) {
			...
			if (drv->nlmode == NL80211_IFTYPE_STATION &&
			    ...
			    0) {
				wpa_msg(drv->ctx, MSG_DEBUG, "nl80211: Unknown BSSID"
				   ...
				   int local_state_change)
{
	...
	genlmsg_put(msg, 0, 0, genl_family_get_id(drv->global->nl80211), 0, 0, cmd, 0);
					 ...
					 int dtim_period, int beacon_int)
{
	...
	wpa_msg(drv->ctx, MSG_DEBUG, "nl80211: Set beacon (beacon_set=%d)",
		beacon_set);
	if (beacon_set)
	...
	ret = send_and_recv_msgs(drv, msg, NULL, NULL);
	if (ret) {
		wpa_msg(drv->ctx, MSG_DEBUG, "nl80211: Beacon set failed: %d (%s)",
			ret, strerror(-ret));
	} else {
		bss->beacon_set = 1;
	}
...
{
	if (params->p2p)
		wpa_msg(drv->ctx, MSG_DEBUG, "nl80211: Setup AP operations for P2P "
			"group (GO)");
				   ...
				   struct wpa_driver_associate_params *params)
{
	...
	if (wpa_driver_nl80211_set_mode(&drv->first_bss, params->mode)) {
		wpa_msg(drv->ctx, MSG_INFO, "nl80211: Failed to set interface into "
			"IBSS mode");
		return -1;
	}
	...
	struct wpa_driver_associate_params *params)
{
	...
	if (params->wpa_ie && params->wpa_ie_len) {
		...
		wpa_msg(drv->ctx, MSG_DEBUG, "  * WPA Version %d", ver);
	...
	msg = NULL;
	if (ret) {
		wpa_msg(drv->ctx, MSG_DEBUG, "nl80211: MLME connect failed: ret=%d "
			"(%s)", ret, strerror(-ret));
		goto nla_put_failure;
	}
	ret = 0;
		       ...
		       struct wpa_init_params *params)
{
	...
	if (wpa_driver_nl80211_set_mode(bss, IEEE80211_MODE_AP)) {
		wpa_msg(drv->ctx, MSG_ERROR, "nl80211: Failed to set interface %s "
				  ...
				  u64 *cookie_out)
{
	...
	msg = NULL;
	if (ret) {
		wpa_msg(drv->ctx, MSG_DEBUG, "nl80211: Frame command failed: ret=%d "
			"(%s)", ret, strerror(-ret));
		goto nla_put_failure;
	}
...
static int wpa_driver_nl80211_probe_req_report(void *priv, int report)
{
	...
	if (drv->nlmode != NL80211_IFTYPE_STATION) {
		wpa_msg(drv->ctx, MSG_DEBUG, "nl80211: probe_req_report control only "
	...
	if (! drv->global->nl_handle_preq) {
		drv->global->nl_handle_preq = nl80211_handle_alloc(drv->global->nl_cb);
		if (drv->global->nl_handle_preq == NULL) {
			wpa_msg(drv->ctx, MSG_ERROR, "nl80211: Failed to allocate "
				"netlink callbacks (preq)");
			goto out_err1;
		}

		if (genl_connect(drv->global->nl_handle_preq)) {
...
#ifdef CONFIG_LIBNL20
		if (genl_ctrl_alloc_cache(drv->global->nl_handle_preq,
...
#else /* CONFIG_LIBNL20 */
		drv->global->nl_cache_preq = genl_ctrl_alloc_cache(drv->global->nl_handle_preq);
		...
		if (nl80211_register_frame(drv, drv->global->nl_handle_preq,
				   (WLAN_FC_TYPE_MGMT << 2) |
		...
		eloop_register_read_sock(nl_socket_get_fd(drv->global->nl_handle_preq),
					 wpa_driver_nl80211_event_receive, drv->global,
					 drv->global->nl_handle_preq);
 ...
 out_err2:
	nl80211_handle_destroy(drv->global->nl_handle_preq);
	drv->global->nl_handle_preq = NULL;
