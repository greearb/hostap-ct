...
static int wpas_sta_secondary_channel_offset(struct wpa_supplicant *wpa_s, struct wpa_bss *bss,
					     ...
					     u8 *channel)
{
	...
	return wpas_get_op_chan_phy(wpa_s, ssid, bss->freq, ies, ies_len, current,
				    channel, &phy_type);
}
			     ...
			     struct wpa_bss *bss, u8 *pos, size_t len)
{
	 ...
	 * or used.
	 */
	if (wpas_sta_secondary_channel_offset(wpa_s, bss, ssid, &current, &chan) < 0 &&
