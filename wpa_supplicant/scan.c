...
static void wpa_supplicant_gen_assoc_event(struct wpa_supplicant *wpa_s)
{
	...
	wpa_msg(wpa_s, MSG_DEBUG, "Already associated with a configured network - "
				     ...
				     struct wpa_ssid *ssid)
{
	...
	if (ssid == NULL) {
		wpa_msg(wpa_s, MSG_DEBUG, "wpa_supplicant_scan: Reached "
...
static void wpa_supplicant_scan(void *eloop_ctx, void *timeout_ctx)
{
	...
	if (wpa_s->wpa_state == WPA_INTERFACE_DISABLED) {
		wpa_msg(wpa_s, MSG_DEBUG, "Skip scan - interface disabled");
		return;
	}
	...
	if (!wpa_supplicant_enabled_networks(wpa_s->conf) &&
	    !wpa_s->scan_req) {
		wpa_msg(wpa_s, MSG_DEBUG, "No enabled networks - do not scan");
	...
	if (wpa_s->conf->ap_scan != 0 &&
		...
		wpa_msg(wpa_s, MSG_DEBUG, "Using wired authentication - "
	...
	if (ssid) {
		wpa_s->prev_scan_ssid = ssid;
		if (max_ssids > 1) {
			wpa_msg(wpa_s, MSG_DEBUG, "Include wildcard SSID in the "
				"scan request");
			params.num_ssids++;
		}
		wpa_msg(wpa_s, MSG_DEBUG, "Starting AP scan for specific SSID(s)");
	} else {
		wpa_s->prev_scan_ssid = WILDCARD_SSID_SCAN;
		params.num_ssids++;
		wpa_msg(wpa_s, MSG_DEBUG, "Starting AP scan for wildcard SSID");
	}
	...
	if (params.freqs == NULL && wpa_s->p2p_in_provisioning &&
		...
		if (wpa_s->p2p_in_provisioning < 5 &&
			...
			wpa_msg(wpa_s, MSG_DEBUG, "P2P: Scan only GO preferred "
				"frequency %d MHz",
			...
			wpa_msg(wpa_s, MSG_DEBUG, "P2P: Scan only common "
				"channels");
			int_array_concat(&params.freqs,
	...
	if (params.freqs == NULL && wpa_s->after_wps && wpa_s->wps_freq) {
		 ...
		 * during provisioning.
		 */
		wpa_msg(wpa_s, MSG_DEBUG, "WPS: Scan only frequency %u MHz that "
	...
	if (params.freqs == NULL && wpa_s->next_scan_freqs) {
		wpa_msg(wpa_s, MSG_DEBUG, "Optimize scan based on previously "
				...
				struct scan_info *info, int new_scan)
{
	...
	if (scan_res == NULL) {
		wpa_msg(wpa_s, MSG_DEBUG, "Failed to get scan results");
		return NULL;
	}

#ifdef CONFIG_WPS
	if (wpas_wps_in_progress(wpa_s)) {
		wpa_msg(wpa_s, MSG_DEBUG, "WPS: Order scan results with WPS "
			"provisioning rules");
