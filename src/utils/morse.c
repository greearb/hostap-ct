...
static bool morse_s1g_op_class_has_cc(const struct ah_class *class, const char *cc)
{
	...
	while (i < (int)ARRAY_SIZE(class->cc_list)) {
...
int morse_is_s1g_freq(struct hostapd_freq_params *params) {
	return (params->center_freq1 >  MIN_S1G_FREQ_KHZ &&
		params->center_freq1 < MAX_S1G_FREQ_KHZ &&
		params->freq >  MIN_S1G_FREQ_KHZ &&
		params->freq < MAX_S1G_FREQ_KHZ);
}
#endif /* CONFIG_IEEE80211AH */
